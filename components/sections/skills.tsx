        ...
        <div className="mt-12 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {skills.map((skill, index) => (
            <div
              ...
              className={`flex flex-col items-center gap-3 rounded-lg border border-border bg-card p-6 transition-all hover:border-primary hover:-translate-y-1 ${isVisible ? "animate-scale-in" : "scroll-hidden"}`}
              style={{ "--stagger-delay": `${Math.min(index * 80, 800)}ms` } as React.CSSProperties}
            >
